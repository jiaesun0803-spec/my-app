        ...
        if not st.session_state["api_key"]:
            ...
            try:
                with st.status("🚀 잼(Jam)이 시각화 리포트를 생성 중입니다...", expanded=True) as status:
                    ...
                    2. 내용 풍성하게: 외부 지식을 동원하여 각 항목을 구체적이고 풍성하게 작성하세요.
                    3. 마침표 줄바꿈: 문장이 마침표('.')로 끝날 때마다 반드시 줄바꿈 문자(<br>)를 추가하세요.
                    4. 기호 삽입: 3, 4, 5, 6, 9번 항목은 반드시 모든 문장 앞에 '-' 기호를 붙이세요.
                    ...
                    (매출 숫자는 기재하지 말고 향후 긍정적인 기대감을 심어주는 코멘트 명사형 3~4줄 작성, 마침표 뒤 줄바꿈)
                    ...
                    <style>
                        * {{ box-sizing: border-box; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }}
                        ...
                        @media print {{ 
                            ...
                            h1 {{ margin: 0 0 15px 0 !important; font-size: 28px !important; padding: 0 !important; }}
                            ...
                            table {{ font-size: 14px !important; margin-bottom: 10px !important; }}
        ...
        if not st.session_state["api_key"]:
            ...
            try:
                with st.status("🚀 잼(Jam)이 기관별 컷오프 및 한도 구간을 심사 중입니다...", expanded=True) as status:
                    ...
                    # 2. [수정] 한도 절대 강제 및 포맷팅(내용 추가, 줄바꿈) 완벽 통제 프롬프트
                    prompt = f"""
                    ...
                    2. 글머리 기호 및 줄바꿈: 2, 3, 4번 항목의 **모든 문장 앞에는 반드시 '-' 기호를 붙이고, 문장이 마침표('.')로 끝날 때마다 무조건 HTML 태그 `<br>`을 삽입하여 줄바꿈** 하세요. (예: - ~하였습니다.<br>- ~필요합니다.<br>)
                    3. 내용 풍성하게: 외부 지식을 동원하여 각 기관별 추천 사유, 합격 전략, 보완 조언을 3~4문장 이상으로 상세하고 풍성하게 채워주세요. 내용이 짧으면 절대 안 됩니다.

                    [절대 매칭 비법 DB - 한도 및 컷오프 룰 (가장 중요!)]
                    1. 기관별 절대 한도 제한 (이 한도 수치를 어기면 안 됨!)
                       - 소진공(신용취약소상공인자금): 최대 3,000만 원까지만 한도 배정 (NICE 839점 이하만 추천)
                       - 신보(신용보증기금) 및 기보(기술보증기금): 최소 1억 원 이상부터 융자 가능 (산출 한도가 1억 미만이면 추천 목록에서 무조건 제외!)
                       - 지역신용보증재단: 최대 2억 원까지만 한도 배정
                       - 중진공(직접대출): 최소 5,000만 원 이상부터 한도 배정
                    2. 신보/기보 한도 산출 공식: 제조업은 '금년 매출의 1/4', 기타 업종은 '금년 매출의 1/6~1/10'로 계산하되, 여기서 반드시 [총 기대출({total_debt})]을 차감한 금액만 한도로 제시할 것. (차감 후 1억 미만이면 신보/기보 탈락)
                    3. 보증기관 우선순위 룰: '기보' 또는 '신보'를 1~2순위에 배치하고, '지역신보'는 3~4순위로 미룰 것.
                    4. 중복 금지: 기보 대출이 있으면 신보 추천 금지, 신보 대출이 있으면 기보 추천 금지.
                    5. 세금체납({tax_status}), 금융연체({fin_status})가 '유'인 경우: 모든 자금 추천을 중지하고 연체 해소 전략만 출력.
                    ...
                    (데이터를 바탕으로 정책자금 합격 가능성에 대한 팩트폭격 스펙 평가. 풍성하게 3~4줄 작성, 문장마다 마침표 뒤 줄바꿈)
                ...
                # [수정] 폰트 축소 완전히 제거, 웹 화면의 큰 카테고리 글씨(h2)가 그대로 PDF로 박히게 함.
                html_export = f"""
                <!DOCTYPE html>
                <html>
                <head>
                    ...
                    <style>
                        * {{ box-sizing: border-box; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }}
                        ...
                        @media print {{ 
                            .print-btn {{ display: none; }} 
                            @page {{ size: A4; margin: 10mm; }}
                            /* 글자 크기 강제 축소 코드 싹 제거! 화면과 동일하게 시원한 폰트 크기 유지 */
                            body {{ padding: 0 !important; font-size: 16px !important; color: black !important; }} 
                            h1 {{ margin: 0 0 15px 0 !important; font-size: 28px !important; padding: 0 !important; }}
                            h2 {{ margin: 25px 0 10px 0 !important; font-size: 24px !important; padding-bottom: 5px !important; border-bottom: 2px solid #174EA6 !important; }}
                            div {{ padding: 15px !important; margin-bottom: 15px !important; border-radius: 10px !important; page-break-inside: avoid; }}
