...
if st.session_state["view_mode"] == "INPUT":
    ...
    with c3_col1:
        ...
        with r2_c1: s_kcb = st.number_input("k_in", value=st.session_state.get("in_kcb_score", None), key="in_kcb_score", label_visibility="collapsed", placeholder=GUIDE)
        with r2_c2: s_nice = st.number_input("n_in", value=st.session_state.get("in_nice_score", None), key="in_nice_score", label_visibility="collapsed", placeholder=GUIDE)
    with c3_col2:
        st.markdown("<div style='margin-top: 15px;'></div>", unsafe_allow_html=True)
    ...
    # 요청대로 아래 선(---) 제거 및 약간의 간격 추가
    st.markdown("<div style='margin-top: 10px;'></div>", unsafe_allow_html=True)
    
    # 일반 매출 현황 (Placeholder 적용)
    ...
    for row_idx in range(0, len(debt_items), 4):
        ...
        for col_idx in range(4):
            ...
            if item_idx < len(debt_items):
                ...
                # value=None으로 명시해야 Placeholder(흐릿한 문구)가 나타납니다.
    ...
    st.success("✅ [수정 완료] Placeholder 가이드 적용 및 수출현황 정렬이 완료되었습니다.")
...
# ==========================================
    ...
    if st.session_state["view_mode"] == "REPORT":
        ...
        with st.status("🚀정밀 분석 중..."):
            res = clean_html(model.generate_content(f"{cn} 기업 리포트 HTML").text)
