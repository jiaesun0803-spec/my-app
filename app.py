...
if check_password():
    ...
    if st.sidebar.button("📊 1. AI기업분석리포트", key="side_1", use_container_width=True):
        if st.session_state.get("in_company_name", "").strip(): change_mode("REPORT"); st.rerun()
        else: st.sidebar.error("🚨 기업명을 입력해주세요.")
    if st.sidebar.button("💡 2. AI 정책자금 매칭리포트", key="side_2", use_container_width=True):
        if st.session_state.get("in_company_name", "").strip(): change_mode("MATCHING"); st.rerun()
        else: st.sidebar.error("🚨 기업명을 입력해주세요.")
    if st.sidebar.button("📝 3. 기관별 융자/사업계획서", key="side_3", use_container_width=True):
    ...
    if st.session_state["view_mode"] == "INPUT":
        ...
        with c3:
            ...
            if has_add_biz == "유": st.text_input("추가 정보", key="in_additional_biz_addr")
        ...
        with m3: st.number_input("중진공 부채(만원)", value=0, key="in_debt_kosme")
        with m4: st.number_input("신보/기보 부채(만원)", value=0, key="in_debt_kodit")
        ...
        with ac2:
            st.checkbox("이노비즈", key="in_chk_4"); st.checkbox("뿌리기업확인서", key="in_chk_7"); st.checkbox("ISO/HACCP", key="in_chk_10")
        ...
        with pat_col1:
            ...
            if has_pat == "유":
                st.text_input("특허출원/등록 건수", key="in_pat_reg")
        with pat_col2:
        ...
        st.text_area("판매루트 및 향후계획", key="in_future_plan")
        ...
        if st.session_state["view_mode"] == "REPORT":
            ...
            if "generated_report" not in st.session_state:
                with st.status("🚀기업분석리포트는 생성중입니다", expanded=True):
                    ...
                    prompt = f"""당신은 전문 경영컨설턴트입니다. 아래 기업의 분석 리포트를 HTML 형식으로 작성하세요.
                    반드시 최상단에 [기업현황표]를 HTML <table>로 포함하세요.
                    - 기업명: {c_name} | 사업자번호: {biz_no} (법인: {corp_no})
                    - 기업형태: {biz_type_str} | 주소: {addr}
                    리포트에는 신청자금 {req_fund_str}의 활용 방안과 아이템 및 시장 분석을 포함하세요."""
            ...
            if "generated_matching" not in st.session_state:
                with st.status("🚀전년도 매출 기준으로 심사를 진행 중입니다", expanded=True):
                    model = genai.GenerativeModel(get_best_model_name())
                    prompt = f"정책자금 전문가로서 {c_name}(매출 {format_kr_currency(d.get('in_sales_current', 0))})에게 맞는 자금 1~4순위를 추천하는 HTML 리포트를 작성해줘."
            ...
            with tabs[0]: 
                if st.button("🚀 중진공 서식 생성"):
                    with st.status("서식 작성 중..."):
                        model = genai.GenerativeModel(get_best_model_name())
                        st.session_state["kosme_html"] = clean_html(model.generate_content(f"{c_name} 중진공 양식").text)
            ...
            with tabs[1]:
                ...
                if st.button("🚀 소진공 서식 생성"):
                    with st.status("서식 작성 중..."):
                        model = genai.GenerativeModel(get_best_model_name())
                        st.session_state["semas_html"] = clean_html(model.generate_content(f"{c_name} 소진공 {semas_fund} 양식").text)
