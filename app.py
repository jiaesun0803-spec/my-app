...
# [실시간 자동 저장 함수]
def auto_save():
    # 'in_'으로 시작하는 모든 세션 데이터를 수집하여 파일에 즉시 저장
...
if st.session_state["company_list"]:
    target = st.sidebar.selectbox("업체 선택", options=["선택 안함"] + list(st.session_state["company_list"].keys()))
...
if st.session_state["view_mode"] == "INPUT":
    # --- 1. 기업현황 (요청하신 크기 통일 적용) ---
    st.header("1. 기업현황")
    # [수정] 기업명, 사업개시일, 전화번호, 이메일을 동일 크기로 배치
    c1r1 = st.columns([1, 1, 1, 1])
    with c1r1[0]: st.text_input("기업명", key="in_company_name", placeholder="업체명을 입력하세요", on_change=auto_save)
    with c1r1[1]: st.text_input("사업개시일", key="in_start_date", placeholder="YYYY.MM.DD", on_change=auto_save)
    with c1r1[2]: st.text_input("사업장 전화번호", key="in_biz_tel", placeholder="000-00-0000", on_change=auto_save)
    with c1r1[3]: st.text_input("이메일 주소", key="in_email_addr", placeholder="example@email.com", on_change=auto_save)
    
    c1r2 = st.columns([1, 1.5, 1.5])
    with c1r2[0]: st.radio("사업자유형", ["개인", "법인"], horizontal=True, key="in_biz_type", on_change=auto_save)
    with c1r2[1]: st.text_input("사업자번호", key="in_raw_biz_no", placeholder="000-00-00000", on_change=auto_save)
    with c1r2[2]: st.text_input("법인등록번호", key="in_raw_corp_no", placeholder="000000-0000000", on_change=auto_save)
    
    c1r3 = st.columns([2.5, 1.5])
    with c1r3[0]: st.text_input("사업장 주소", key="in_biz_addr", placeholder="소재지를 입력하세요", on_change=auto_save)
    with c1r3[1]: st.selectbox("업종", ["제조업", "서비스업", "IT업", "도소매업", "건설업", "기타"], key="in_industry", on_change=auto_save)
    
    c1r4 = st.columns([1, 1, 1])
    with c1r4[0]: st.radio("사업장 임대여부", ["자가", "임대"], horizontal=True, key="in_lease_status", on_change=auto_save)
    with c1r4[1]: st.number_input("보증금 (만원)", key="in_lease_deposit", step=1, placeholder=GUIDE_STR, value=None, on_change=auto_save)
    with c1r4[2]: st.number_input("월임대료 (만원)", key="in_lease_rent", step=1, placeholder=GUIDE_STR, value=None, on_change=auto_save)
    
    c1r5 = st.columns([1, 2])
    with c1r5[0]: st.radio("추가 사업장 여부", ["없음", "있음"], horizontal=True, key="in_has_extra_biz", on_change=auto_save)
    with c1r5[1]: st.text_input("추가사업장 정보입력", key="in_extra_biz_info", placeholder="사업장명/사업자등록번호", on_change=auto_save)
    st.markdown("---")
    ...
    with exp_c1: has_exp = st.radio("수출매출 여부", ["없음", "있음"], horizontal=True, key="in_export_revenue", on_change=auto_save)
    with exp_c2: plan_exp = st.radio("수출예정 여부", ["없음", "있음"], horizontal=True, key="in_planned_export", on_change=auto_save)
    mc = st.columns(4)
    ...
    if has_exp == "있음":
        ...
        for i, (t, k) in enumerate(e_keys): ec[i].number_input(f"{t} (만원)", key=k, step=1, placeholder=GUIDE_STR, value=None, on_change=auto_save)
    st.markdown("---")
    ...
    r8_1 = st.columns(2)
    with r8_1[0]: st.text_area("핵심 아이템", key="in_item_desc", height=100, placeholder="제품/서비스 기능", on_change=auto_save)
    with r8_1[1]: st.text_area("판매 루트(유통망)", key="in_sales_route", height=100, placeholder="주요 거래처", on_change=auto_save)
    r8_2 = st.columns(2)
    with r8_2[0]: st.text_area("경쟁력 및 차별성", key="in_item_diff", height=100, placeholder="타사 대비 강점", on_change=auto_save)
    with r8_2[1]: st.text_area("시장 현황", key="in_market_status", height=100, placeholder="업계 분위기", on_change=auto_save)
    r8_3 = st.columns(2)
    with r8_3[0]: st.text_area("공정도", key="in_process_desc", height=100, placeholder="생산 과정 요약", on_change=auto_save)
    with r8_3[1]: st.text_area("타겟 고객", key="in_target_cust", height=100, placeholder="주요 구매층", on_change=auto_save)
    r8_4 = st.columns(2)
    with r8_4[0]: st.text_area("수익 모델", key="in_revenue_model", height=100, placeholder="매출 발생 구조", on_change=auto_save)
    with r8_4[1]: st.text_area("앞으로의 계획", key="in_future_plan", height=100, placeholder="향후 비즈니스 목표", on_change=auto_save)
    st.markdown("---")
    ...
    if not final_api_key:
        ...
        with st.status(f"🚀 {biz_name} AI 분석 중..."):
            try:
                # engine_analysis.py 등에서 모델명을 gemini-1.5-flash-latest로 호출해야 함
