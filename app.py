...
if check_password():
    ...
    # ==========================================
    # 1. 사이드바 (API 설정 및 업체 관리)
    # ==========================================
    st.sidebar.header("⚙️ Gemini AI 설정")
    api_key_input = st.sidebar.text_input("Gemini API Key", type="password", key="gemini_api_key")
    ...
    if st.session_state["view_mode"] == "REPORT":
        ...
        if not api_key_input:
            ...
            try:
                with st.status("🚀 AI 전문가 분석 중...", expanded=True) as status:
                    # [핵심] 404 에러 해결을 위한 모델명 지정
                    model = genai.GenerativeModel('models/gemini-1.5-flash')
                    
                    prompt = f"""
                    전문 컨설턴트로서 다음 데이터를 분석해 9개 항목 리포트를 작성하세요.
                    ...
                    아이템: {d.get('in_item_desc')}, 자금용도: {d.get('in_fund_purpose')}
                    구조: 1.기업현황분석 2.SWOT 3.시장현황 4.경쟁력분석 5.정책자금추천 6.인증제안 7.사용계획 8.매출전망 9.성장비전
                    """
    ...
    # --- [입력 화면 모드] ---
        ...
        with r1:
            ...
            with rc2: st.text_input("생년월일", placeholder="800101", key="in_rep_dob")
        ...
        with cr2:
            ...
            st.markdown(f"""<div style="background-color:#f8f9fa; padding:20px; border-radius:10px; border-left:5px solid #174EA6;">
                <h4 style="margin:0;">🏆 신용등급 판정 결과</h4>
                <p>KCB: <b>{kg}등급</b> / NICE: <b>{ng}등급</b></p></div>""", unsafe_allow_html=True)
        ...
        with m1: st.number_input("금년 매출", key="in_sales_current")
        with m2: st.number_input("25년 매출", key="in_sales_2025")
        with m3: st.number_input("24년 매출", key="in_sales_2024")
        with m4: st.number_input("23년 매출", key="in_sales_2023")
        ...
        with d3: st.number_input("신보재단", key="in_debt_koreg")
        d4, d5, d6, d7, d8 = st.columns(5)
        with d4: st.number_input("신보", key="in_debt_kodit")
        with d5: st.number_input("기보", key="in_debt_kibo")
