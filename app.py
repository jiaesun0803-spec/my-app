...
if st.session_state["view_mode"] == "INPUT":
    ...
    # [보완] 라벨 폰트 크기를 통일하여 입력창 수평 라인을 맞춤
    ...
    with c3_col2:
        ...
        if delinquency == "유" or tax_delin == "유":
            st.markdown(f"""<div style='background-color:#FFEBEE; padding:20px; border-radius:10px; height:185px;'><p style='color:#B71C1C; font-weight:700;'>⚠️ 연체/체납 확인</p><p style='font-size:0.9em;'>자금 신청 전 즉시 소명 및 해결이 필요합니다.</p></div>""", unsafe_allow_html=True)
        else:
            st.markdown(f"""<div style='background-color:#E8F5E9; padding:20px; border-radius:10px; height:185px;'><p style='color:#1B5E20; font-weight:700;'>✅ 신용 양호</p><p style='font-size:0.9em;'>기초 신용 요건을 충족하고 있습니다.</p></div>""", unsafe_allow_html=True)
    with c3_col3:
    ...
    # --- 8. 비즈니스 상세 정보 (AI 기능 포함) ---
    st.markdown("<br>", unsafe_allow_html=True)
    st.header("8. 비즈니스 상세 정보")
    if st.button("🪄 핵심아이템 기반 AI 자동 초안 생성", type="secondary"):
        if not st.session_state.get("in_item_desc"): st.warning("핵심 아이템 내용을 입력해주세요.")
    ...
    c9 = st.columns([1, 2])
    with c9[0]:
        st.markdown('<p class="blue-text">이번 조달 필요 자금 (만원)</p>', unsafe_allow_html=True)
    ...
    st.success("✅ [보완 완료] 기업현황 라인 정렬, 신용등급 복구 및 명칭 동기화가 완료되었습니다.")
...
# ==========================================
    ...
    with st.status("🚀 AI 분석 진행 중..."):
        model = genai.GenerativeModel(get_best_model_name())
        res = model.generate_content(f"기업 정보: {d} 를 바탕으로 정책자금용 리포트/계획서를 HTML로 작성하라.").text
