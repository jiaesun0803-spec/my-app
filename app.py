...
# 무한 로딩의 주범이었던 list_models() 검색을 삭제하고, 가장 안정적이고 빠른 모델로 직결(하드코딩)
...
if check_password():
    ...
    if st.sidebar.button("📊 1. AI기업분석리포트", use_container_width=True):
        if not st.session_state.get("in_company_name", "").strip():
            st.sidebar.error("🚨 기업명을 입력해야 리포트 생성이 가능합니다.")
        else:
    ...
    if not pat_str: pat_str = "특허/지재권 없음"
    ...
    if st.session_state["view_mode"] == "REPORT":
        ...
        if not st.session_state["api_key"]:
            ...
            try:
                ...
                res_text = st.session_state.get("generated_report", "")

                if "[GRAPH_INSERT_POINT]" in res_text:
                    parts = res_text.partition("[GRAPH_INSERT_POINT]")
                ...
                <body>
                    ...
                    {res_text.replace('[GRAPH_INSERT_POINT]', plotly_html)}
                </body>
                </html>
                """
    ...
    # ---------------------------------------------------------
        ...
        if not st.session_state["api_key"]:
            ...
            try:
                if "generated_matching" not in st.session_state:
                    with st.status("🚀전년도 매출 기준으로 심사를 진행 중입니다", expanded=True) as status:
                        try:
                            ...
                            [입력] 기업명:{c_name} / 업종:{c_ind} / 상시근로자수:{v.emp_cnt}명 / 전년매출:{v.s_25} / 총기대출:{v.tot_debt} / 인증현황:{v.cert_status} / 특허현황:{v.pat_str} / 희망 필요자금:{v.req_fund}
                            ...
                            4. 보완 조언 룰 (인증/특허 활용):
                               - 이 기업의 인증({v.cert_status}) 및 특허 현황({v.pat_str})을 바탕으로, 추가적인 자금 조달처나 금리 우대 혜택 등을 방대한 외부 데이터를 검색/참고하여 컨설팅할 것.
                            ...
                            <div style="background-color:#f8f9fa; padding:20px; border-radius:15px; border:1px solid #e0e0e0; margin-bottom:15px;">
                              <b>기업명:</b> {c_name} | <b>업종:</b> {c_ind} ({biz_type}) | <b>업력:</b> 약 {v.biz_years}년 | <b>상시근로자:</b> {v.emp_cnt}명 <br>
                              <b>NICE 점수:</b> {v.nice_score}점 | <b>기술/벤처 인증:</b> {v.cert_status} | <b>특허정보:</b> {v.pat_str} <br>
                ...
                res_text = st.session_state.get("generated_matching", "")
                st.markdown(res_text, unsafe_allow_html=True)
    ...
    # ---------------------------------------------------------
        ...
        if st.button("⬅️ 대시보드로 돌아가기"):
            for k, val in st.session_state["permanent_data"].items():
        ...
        with tabs[0]:
                ...
                if st.button("🚀 중진공 융자신청서(공통) 바로보기", use_container_width=True):
                    with st.status("🚀 융자신청서(공통양식) 빈칸을 완벽하게 채우는 중입니다...", expanded=True) as status:
                        try:
                            ...
                            [기업 데이터]
                            - 기업명: {v.c_name} / 대표자: {v.rep_name} / 사업자유형: {v.biz_type} / 업종: {v.c_ind}
                            - 본사주소: {v.address} / 학력: {v.edu_school} {v.edu_major} / 자택: {d.get('in_home_addr', '')}
                            - 매출: 23년({v.sales_23}), 24년({v.sales_24}), 금년({v.s_cur})
                            - 수출여부: (데이터 기반)
                            - 신청자금: {v.req_fund} ({v.fund_type} / {v.fund_purpose})
                            - 아이템: {v.item} / 시장: {v.market} / 차별성: {v.diff} / 판매루트: {v.route}
                            - 기초 공정: {v.process_desc}
                            ...
                            <tr><th style="border:1px solid #333; padding:8px; background:#f0f0f0;">신청자금명</th><td style="border:1px solid #333; padding:8px;">▣ {kosme_fund_type}</td><th style="border:1px solid #333; padding:8px; background:#f0f0f0;">신청금액</th><td style="border:1px solid #333; padding:8px;">{v.req_fund}</td></tr>
                            ...
                            <tr><th style="border:1px solid #333; padding:8px; background:#f0f0f0;">소재지</th><td colspan="3" style="border:1px solid #333; padding:8px; text-align:left;">본사: {v.address}</td></tr>
                            <tr><th style="border:1px solid #333; padding:8px; background:#f0f0f0;">기업주 성명</th><td style="border:1px solid #333; padding:8px;">{v.rep_name}</td><th style="border:1px solid #333; padding:8px; background:#f0f0f0;">직위</th><td style="border:1px solid #333; padding:8px;">대표</td></tr>
                            <tr><th style="border:1px solid #333; padding:8px; background:#f0f0f0;">학력</th><td style="border:1px solid #333; padding:8px;">{v.edu_school} {v.edu_major}</td><th style="border:1px solid #333; padding:8px; background:#f0f0f0;">주택</th><td style="border:1px solid #333; padding:8px;">{d.get('in_home_addr', '')}</td></tr>
                            ...
                            <tr><td style="border:1px solid #333; padding:8px;">총매출액</td><td style="border:1px solid #333; padding:8px;">{v.sales_23}</td><td style="border:1px solid #333; padding:8px;">{v.sales_24}</td><td style="border:1px solid #333; padding:8px;">{v.s_cur}</td><td style="border:1px solid #333; padding:8px; color:blue; font-weight:bold;">(자동계산)</td><td style="border:1px solid #333; padding:8px; color:blue; font-weight:bold;">(자동계산)</td></tr>
                            </table>
                            ...
                            st.session_state["kosme_result_html"] = clean_html(response.text)
                    ...
                    with st.status(f"🚀 '{kosme_fund_type}' 전용 1타 심사역 로직으로 작성 중입니다...", expanded=True) as status:
                        try:
                            ...
                            if kosme_fund_type == "청년전용창업자금":
                                ...
                                [기업데이터]
                                - 기업명: {v.c_name} / 대표자: {v.rep_name} / 업력: {v.biz_years}년 / 경력: {v.career}
                                - 아이템: {v.item} / 시장현황: {v.market} / 경쟁우위: {v.diff}
                                ...
                                (대표자의 뼈저린 현장 경험, 시장의 거시적 문제점, 타겟 고객의 미충족 수요를 엮어서 최소 4~5개의 거대한 문단으로 나누어, 1000자 이상의 압도적인 분량으로 매우 깊이 있게 서술.)
                                </td>
                                </tr>
                                <tr>
                                ...
                                [기업데이터]
                                - 기업명: {v.c_name} / 대표자: {v.rep_name} / 업력: {v.biz_years}년
                                - 아이템: {v.item} / 시장현황: {v.market} / 경쟁우위: {v.diff}
                                - 특허/인증: {v.cert_status} / {v.pat_str}
                                - 매출현황: 금년 {v.s_cur}
                                ...
                                <td style="border:1px solid #333; padding:10px;">(제품명) {v.item}</td>
                                </tr>
                                <tr>
                                <td style="border:1px solid #333; padding:10px; line-height:1.6;">- R&D성공과제/특허기술 등: {v.pat_str}<br>- 관련기술 평가/인증 년월일: (가상 작성)</td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                <td style="border:1px solid #333; padding:8px; line-height:1.4;">{v.item}</td>
                            ...
                            fig.add_trace(go.Scatter(
                                x=monthly_labels, y=monthly_vals, mode='lines+markers+text',
                                text=[format_kr_currency(val) for val in monthly_vals], textposition="top center",
                                textfont=dict(size=11), line=dict(color='#1E88E5', width=4, shape='spline'),
                                marker=dict(size=10, color='#FF5252', line=dict(width=2, color='white'))
                            ))
                            fig.update_layout(
                            ...
                            st.session_state["kosme_result_html"] = clean_html(response.text).replace('[GRAPH_INSERT_POINT]', f"<div style='text-align:center; page-break-inside:avoid;'><br>{plotly_html}<br></div>")
            ...
            if "kosme_result_html" in st.session_state:
                ...
                html_export = f"<!DOCTYPE html><html><head><meta charset='utf-8'>{PDF_CSS}</head><body><div class='doc'>{st.session_state['kosme_result_html']}</div></body></html>"
        ...
        with tabs[1]:
            ...
            if "semas_result_html" in st.session_state:
                ...
                html_export = f"<!DOCTYPE html><html><head><meta charset='utf-8'>{PDF_CSS}</head><body><div class='doc'>{st.session_state['semas_result_html']}</div></body></html>"
...
# ==========================================
    ...
    st.markdown("<p style='color:#1a73e8; font-weight:bold;'>※ 아래 정보를 입력해 주세요. (기업명은 필수입니다)</p>", unsafe_allow_html=True)
    st.header("1. 기업현황")
    ...
    st.success("✅ 세팅 완료! 상단 또는 좌측 버튼을 클릭해 리포트를 생성하세요.")
