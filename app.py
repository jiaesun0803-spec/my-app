        ...
        if not st.session_state["api_key"]:
            ...
            try:
                with st.status("🚀 잼(Jam)이 기관별 컷오프, 한도, 유동화자금(P-CBO) 예외 룰을 철저히 심사 중입니다...", expanded=True) as status:
                    ...
                    # 2. [완벽 제어 프롬프트] 신보/기보 2순위 절대 고정 룰 추가
                    prompt = f"""
                    ...
                    4. 내용 분량: A4 1장에 완벽하게 들어갈 수 있도록 각 세부 항목(추천사유, 합격꿀팁 등)은 무조건 외부지식을 동원해 3~4줄로 상세하게 적되 불필요한 서론은 빼세요.
                    ...
                    2. 🥈 2순위 지정 (메이저 보증 - 신보/기보 무조건 고정!!): **2순위 자리는 무조건 '신용보증기금(신보)' 또는 '기술보증기금(기보)'을 배정하세요.** - [신보/기보 한도 산출 공식 및 1억 컷오프 - 경고!!!]: **제조업은 '매출액의 1/2', 그 외 업종은 '매출액의 1/10'** 수준으로 계산합니다. 이 계산된 금액에서 **총 기대출({total_debt})을 반드시 빼서 남은 금액**이 예상 한도입니다! 
                       - **만약 차감 후 계산된 한도가 1억 원 미만이거나, 애초에 희망자금({fund_req})이 1억 원 미만인 경우에만 예외적으로 신보와 기보를 탈락시키고 2순위에 지역신보를 배정하세요. 그 외에는 무조건 신보/기보가 2순위입니다.**
                       - [매출 4억 룰 - 핵심!]: 1억 컷오프를 통과한 상태에서, 매출액 4억(40,000만 원) 이상이면 지역신보가 아닌 신용보증기금(신보)을 강력하게 추천하세요.
                       - [기보 타겟 및 예외 룰]: 기보는 제조업/IT업 중심이나, 타 업종이라도 '벤처인증'이나 '특허'가 있다면 신보보다 기보를 우선 추천.
                       - [중복 금지]: 신보와 기보는 기금으로서 절대 중복 이용이 불가합니다.
                    3. 🚀 [유동화자금(P-CBO/스케일업) 룰 - 법인전용]: 기존 대출이 꽉 차 일반 보증 한도가 없지만, 성장성이 뚜렷한 '법인기업'({biz_type})의 경우 아래를 추천하세요.
                       - 신보 P-CBO: 업력이 4년 이상인 경우에만 추천 가능.
                       - 중진공 스케일업금융: 업력이 5년 이상이고, 매출이 20억(200,000만 원) 이상인 경우에만 추천 가능.
                    4. 🥉 3~4순위 지정 (플랜 B): 
                       - [보증기관 순서 룰 - 핵심!!!]: 보증기관은 **반드시 '신보/기보 먼저 ➡️ 지역신보 나중' 순서로 진행**해야만 두 기관을 중복 이용할 수 있습니다. (지역신보를 먼저 쓰면 신보/기보 한도가 막혀버려 절대 진행 불가). 만약 1억 컷오프를 통과하여 신보/기보가 2순위로 배정되었다면 지역신보는 무조건 3~4순위로 미루고, 컨설턴트 코멘트에 이 순서의 중요성을 강조하세요.
                       - 4순위는 특화기관 등 배치.
