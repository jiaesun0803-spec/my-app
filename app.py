...
<style>
    ...
    /* 입력창 내부 Placeholder(흐릿한 글씨) 크기 조절 */
    input::placeholder {
        font-size: 0.7em !important;
    }
    /* 숫자 입력창 우측 증감 버튼 숨기기 (깔끔한 디자인 선호 시) */
...
if st.session_state["view_mode"] == "INPUT":
    ...
    with c1r3[2]: st.number_input("보증금 (만원)", value=st.session_state.get("in_lease_deposit", None), key="in_lease_deposit", placeholder=GUIDE_STR)
    with c1r3[3]: st.number_input("월임대료 (만원)", value=st.session_state.get("in_lease_rent", None), key="in_lease_rent", placeholder=GUIDE_STR)
    ...
    with c3_col1:
        ...
        t1_c1.markdown("<p style='font-size:14px;'>금융연체 여부</p>", unsafe_allow_html=True)
        t1_c2.markdown("<p style='font-size:14px;'>세금체납 여부</p>", unsafe_allow_html=True)
        ...
        t2_c1.markdown("<p style='font-size:14px;'>KCB 점수</p>", unsafe_allow_html=True)
        t2_c2.markdown("<p style='font-size:14px;'>NICE 점수</p>", unsafe_allow_html=True)
        st.markdown("<div style='margin-top: 10px;'></div>", unsafe_allow_html=True)
    ...
    with c3_col2:
        ...
        if delinquency == "유" or tax_delin == "유":
            st.markdown(f"""<div style='background-color: #FFEBEE; padding: 20px; border-radius: 10px; border-left: 5px solid #E53935; height: 180px; overflow: hidden;'><p style='color: #B71C1C; margin-top: 0; font-weight:bold;'>⚠️ 연체/체납 주의</p><p style='color: #D32F2F; font-size: 0.9em; line-height: 1.5;'>즉시 해결이 필요합니다.</p></div>""", unsafe_allow_html=True)
        else:
            st.markdown(f"""<div style='background-color: #E8F5E9; padding: 20px; border-radius: 10px; border-left: 5px solid #43A047; height: 180px; overflow: hidden;'><p style='color: #1B5E20; margin-top: 0; font-weight:bold;'>✅ 신용 양호</p><p style='color: #2E7D32; font-size: 0.9em; line-height: 1.5;'>기초 신용 요건을 충족한 상태입니다.</p></div>""", unsafe_allow_html=True)
    with c3_col3:
        v_cols = st.columns(2)
    ...
    st.markdown("<div style='margin-top: 15px;'></div>", unsafe_allow_html=True)
    m_titles = ["금년 매출합계 (만원)", "25년도 매출합계 (만원)", "24년도 매출합계 (만원)", "23년도 매출합계 (만원)"]
    m_keys = ["in_sales_cur", "in_sales_25", "in_sales_24", "in_sales_23"]
    
    ic = st.columns(4)
    for i, key in enumerate(m_keys): 
        ic[i].number_input(label=m_titles[i], value=st.session_state.get(key, None), key=key, placeholder=GUIDE_STR)
    
    if has_export == "유":
        st.markdown("<div style='margin-top: 15px;'></div>", unsafe_allow_html=True)
        e_titles = ["금년 수출매출 (만원)", "25년도 수출매출합계 (만원)", "24년도 수출매출합계 (만원)", "23년도 수출매출합계 (만원)"]
        e_keys = ["in_exp_cur", "in_exp_25", "in_exp_24", "in_exp_23"]
        eic = st.columns(4)
        for i, key in enumerate(e_keys): 
            eic[i].number_input(label=e_titles[i], value=st.session_state.get(key, None), key=key, placeholder=GUIDE_STR)
    ...
    for row_idx in range(0, len(debt_items), 4):
        ...
        for col_idx in range(4):
            ...
            if item_idx < len(debt_items):
                ...
                cols[col_idx].number_input(title, value=st.session_state.get(key, None), key=key, placeholder=GUIDE_STR)
    ...
    st.success("✅ [수정 완료] 모든 소제목 볼드 해제 및 Placeholder 가이드 폰트 축소가 적용되었습니다.")
