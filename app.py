...
with tab_load:
    if st.session_state["company_list"]:
        ...
        if st.button("데이터 불러오기", use_container_width=True):
            ...
            st.info(f"'{target}' 로드 완료.")
            st.rerun()
    else:
        st.write("저장된 업체 정보가 없습니다.")
...
if st.session_state["view_mode"] == "INPUT":
    ...
    # 4층: 추가사업장 여부, 추가사업장 정보입력, 현 사업장 업종 (명칭 변경)
    ...
    with c3_col1:
        ...
        with l_r1_c1: st.radio("금융연체여부", ["없음", "있음"], horizontal=True, key="in_fin_delinquency")
        with l_r1_c2: st.radio("세금체납여부", ["없음", "있음"], horizontal=True, key="in_tax_delinquency")
    ...
    with c7[0]:
        ...
        st.text_area("특허 상세 내용", key="in_pat_desc", placeholder="특허 명칭 및 내용")
    with c7[1]:
        st.radio("정부지원 수혜이력", ["없음", "있음"], horizontal=True, key="in_has_gov")
        st.number_input("수혜 건수", key="in_gov_cnt", step=1, value=None)
        st.text_area("수혜 사업명 상세", key="in_gov_desc", placeholder="과거 선정 사업명")
    st.markdown("---")
    ...
    with r8_1[1]: st.text_area("판매 루트(유통망)", key="in_sales_route", height=100, placeholder="주요 거래처 및 채널")
    r8_2 = st.columns(2)
    with r8_2[0]: st.text_area("경쟁력 및 차별성", key="in_item_diff", height=100, placeholder="타사 대비 강점")
    with r8_2[1]: st.text_area("시장 현황", key="in_market_status", height=100, placeholder="업계 분위기")
    r8_3 = st.columns(2)
    with r8_3[0]: st.text_area("공정도", key="in_process_desc", height=100, placeholder="생산 과정 요약")
    with r8_3[1]: st.text_area("타겟 고객", key="in_target_cust", height=100, placeholder="주요 구매층")
    r8_4 = st.columns(2)
    with r8_4[0]: st.text_area("수익 모델", key="in_revenue_model", height=100, placeholder="매출 발생 구조")
    ...
    if not final_api_key:
        ...
        with st.status(f"🚀 {biz_name} AI 분석 중..."):
            try:
                # engine_analysis.py 등 내부 모델명은 gemini-1.5-flash-latest로 설정 권장
