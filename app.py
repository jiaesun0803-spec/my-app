...
def get_common_vars(d):
    ...
    v.c_name = g('in_company_name').strip()
    ...
    v.add_biz_status, v.add_biz_addr = g('in_has_additional_biz', '무'), g('in_additional_biz_addr')
...
# 탭 이동 로직
def set_mode(m):
    if not str(st.session_state.get("in_company_name", "")).strip():
...
if st.session_state["view_mode"] in ["REPORT", "MATCHING", "PLAN"]:
    ...
    if st.session_state["view_mode"] == "REPORT" and "generated_report" not in st.session_state:
        with st.status("🚀 리포트 생성 중..."):
            try:
                ...
                pr = f"컨설턴트. 마크다운 금지. 파스텔톤 HTML 표. [기업]{v.c_name}/업종:{v.c_ind}/아이템:{v.item}. 시장 동향(1000자 이상) 및 분석 결과 서술. [GRAPH_INSERT_POINT] 필수 포함."
    ...
    res_html = st.session_state.get("generated_report", "심사 중...") if st.session_state["view_mode"] == "REPORT" else "데이터를 처리 중입니다..."
    ...
    st.download_button("📥 리포트 다운로드", final_html, f"{v.c_name}_리포트.html", "text/html")

# ==========================================
# 3. 메인 대시보드 (입력 화면) - 필수사항 해제
# ==========================================
else:
    st.markdown("<p style='color:#1a73e8; font-weight:bold; font-size:1.1em;'>※ 정보를 입력하시면 AI가 맞춤형 리포트를 생성합니다.</p>", unsafe_allow_html=True)
    ...
    with cr2:
        ...
        st.markdown(f"<div style='padding:25px; background:#e8f0fe; border-radius:15px; text-align:center; border:2px solid #a4c2f4; margin-top:10px;'><h3 style='color:#1967d2; margin-top:0; font-size:20px;'>🏆 신용등급 판정 결과</h3><p style='font-size:30px; font-weight:900; margin-bottom:0; color:#111;'>KCB: <span style='color:#d32f2f;'>{kg}등급</span> | NICE: <span style='color:#d32f2f;'>{ng}등급</span></p></div>", unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)
    st.header("4. 매출현황")
    m1, m2, m3, m4 = st.columns(4)
    with m1: st.number_input("금년(당월) 매출(만)", value=0, step=1, format="%d", key="in_sales_current")
    ...
    for i in range(0, 8, 4):
        ...
        for j in range(4):
            idx = i + j
            if st.checkbox(["소상공인", "창업", "여성기업", "이노비즈", "벤처", "뿌리", "ISO", "HACCP"][idx], key=f"in_chk_{idx}"):
                st.text_input(f"↪ 일자", placeholder="YYYYMMDD", key=f"in_cert_date_{idx}", on_change=cb_format_date, args=(f"in_cert_date_{idx}",))
    
    st.success("✅ 세팅 완료! 상단 버튼을 클릭해 리포트를 생성하세요.")

# [작업 완료] 🏁 잼(Jam) 복구 및 전면 개방 완료! 다음 지시를 내려주십시오! 🏁
