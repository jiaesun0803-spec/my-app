...
# 대표님의 기존 원본 방식 (API 환경에 맞춰 자동 검색 및 다운그레이드 지원)
...
if check_password():
    ...
    def check_company_name():
        c_name = st.session_state.get("in_company_name", "").strip()
        if not c_name:
            st.sidebar.error("🚨 최소한 기업명은 입력해야 리포트 생성이 가능합니다.")
            return False
        return True
                            ...
                            for attempt in range(max_retries):
                                try:
                                    ...
                                    if "429" in str(e) and attempt < max_retries - 1:
                                        status.update(label=f"⏳ 구글 서버 지연. 5초 후 재시도합니다... ({attempt+1}/{max_retries})", state="running")
                                        time.sleep(5)
                                    else:
                                        raise e
                        except Exception as e:
