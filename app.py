...
# API KEY 저장 여부에 따른 입력창 토글
if "api_key_saved" not in st.session_state: st.session_state["api_key_saved"] = False

if not st.session_state["api_key_saved"]:
    api_key_input = st.sidebar.text_input("Gemini API Key", type="password", placeholder="AI 엔진 키를 입력하세요")
...
if st.sidebar.button("💾 현재 업체 정보 저장", use_container_width=True):
    ...
    if cn:
        ...
        st.sidebar.warning("기업명을 먼저 입력해주세요.")
...
if st.sidebar.button("📊 AI기업분석리포트", key="sb_report"): st.session_state["view_mode"] = "REPORT"; st.rerun()
if st.sidebar.button("💡 AI 정책자금 매칭", key="sb_matching"): st.session_state["view_mode"] = "MATCHING"; st.rerun()
if st.sidebar.button("📝 융자/사업계획서", key="sb_plan"): st.session_state["view_mode"] = "PLAN"; st.rerun()
...
t_cols = st.columns(4)
with t_cols[0]: 
    if st.button("📊 AI기업분석리포트", use_container_width=True, type="primary", key="main_report"): st.session_state["view_mode"] = "REPORT"; st.rerun()
with t_cols[1]: 
    if st.button("💡 AI 정책자금 매칭", use_container_width=True, type="primary", key="main_matching"): st.session_state["view_mode"] = "MATCHING"; st.rerun()
with t_cols[2]: 
    if st.button("📝 AI 사업계획서", use_container_width=True, type="primary", key="main_plan"): st.session_state["view_mode"] = "PLAN"; st.rerun()
with t_cols[3]: 
    if st.button("📑 데이터 리셋", use_container_width=True, key="main_reset"): 
        for key in list(st.session_state.keys()): 
            if key.startswith("in_"): del st.session_state[key]
        st.rerun()
...
if st.session_state["view_mode"] == "INPUT":
    ...
    with c3_col2:
        ...
        if delinquency == "유" or tax_delin == "유":
            box_html = "<div style='background-color:#FFEBEE; padding:20px; border-radius:10px; height:185px;'><p style='color:#B71C1C; font-weight:700;'>⚠️ 연체/체납 확인</p><p style='font-size:0.9em;'>심사 전 즉시 해결이 필요합니다.</p></div>"
        else:
            box_html = "<div style='background-color:#E8F5E9; padding:20px; border-radius:10px; height:185px;'><p style='color:#1B5E20; font-weight:700;'>✅ 신용 양호</p><p style='font-size:0.9em;'>기초 요건을 충족하고 있습니다.</p></div>"
    ...
    if has_export == "유":
        ...
        for i, key in enumerate(e_keys): 
            eic[i].number_input(f"<span style='color:#1E88E5;'>{e_titles[i]}</span> (만원)", value=st.session_state.get(key, None), key=key, placeholder=GUIDE_STR, step=1, format="%d")
    ...
    with c67[1]:
        ...
        st.number_input("특허 건수", value=st.session_state.get("in_pat_cnt", None), key="in_pat_cnt", step=1, format="%d")
    ...
    if st.sidebar.button("🪄 AI 비즈니스 정보 초안 생성", help="핵심 아이템 기반으로 자동 작성"):
        if not st.session_state.get("in_item_desc"): st.warning("핵심 아이템을 먼저 입력하세요.")
        elif not st.session_state.get("api_key"): st.error("API Key를 먼저 저장하세요.")
        else:
            with st.spinner("AI가 비즈니스 전략 구상 중..."):
                try:
                    model = genai.GenerativeModel(get_best_model_name())
                    fill_prompt = f"아이템: {st.session_state['in_item_desc']}에 대해 차별성, 공정도, 시장현황, 타겟고객, 수익모델, 판매루트, 계획을 JSON 형식으로 작성하라."
                    ...
                    st.rerun()
                except: st.error("AI 초안 생성 실패. 직접 입력을 권장합니다.")
    ...
    st.success("✅ [최종 보완 완료] 모든 기능 복구 및 API 보안 설정이 완료되었습니다.")
...
# ==========================================
    ...
    with st.status("🚀 AI 리포트 생성 중..."):
        if not st.session_state.get("api_key"): st.error("API Key를 먼저 설정하세요.")
        else:
            model = genai.GenerativeModel(get_best_model_name())
            res = model.generate_content(f"기업 정보: {d} 를 바탕으로 정책자금용 분석 보고서를 HTML로 상세히 작성하라.").text
