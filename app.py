...
def get_ai_model():
    # 무한 로딩(Timeout) 및 404 에러를 차단하기 위해 가장 안정적이고 빠른 모델로 명시적 강제 고정
...
def get_common_vars(d):
    ...
    # 누락 방지를 위해 edu/home 추가
    v.edu_school = g('in_edu_school', '')
    v.edu_major = g('in_edu_major', '')
    v.home_addr = g('in_home_addr', '')
    return v
...
if st.session_state["view_mode"] != "INPUT":
    ...
    if st.session_state["view_mode"] == "REPORT":
        ...
        if "generated_report" not in st.session_state:
            with st.status("🚀기업분석리포트는 생성중입니다...") as status:
                try:
                    pr = f"""전문 경영컨설턴트입니다. 
        ...
        st.download_button("📥 리포트 다운로드", html_export, f"{safe_name}_분석리포트.html", "text/html")
        ...
        safe_name = "".join([c for c in v.c_name if c.isalnum() or c in (" ", "_")]).strip() or "업체"
        st.download_button("📥 매칭리포트 다운로드", html_export, f"{safe_name}_매칭리포트.html", "text/html")
        ...
        with tabs[0]:
            ...
            with c_p1:
                if st.button("🚀 중진공 융자신청서(공통) 생성", use_container_width=True):
                    with st.status("🚀 융자신청서 렌더링 중...") as status:
                        try:
                            pr = f"컨설턴트. 마크다운 금지. HTML 표. [기업]{v.c_name}/매출:{v.s_cur}/자금:{v.req_fund}/아이템:{v.item}. <h2 style='text-align:center;'>중소기업 정책자금 융자신청서</h2><table style='width:100%; border:1px solid #333;'><tr><th>신청자금</th><td>▣ {sf}</td><th>금액</th><td>{v.req_fund}</td></tr></table> 공정도와 시장상황 상세히 작성하세요."
            ...
            if "kosme_result_html" in st.session_state:
                ...
                safe_name = "".join([c for c in v.c_name if c.isalnum() or c in (" ", "_")]).strip() or "업체"
                st.download_button("📥 중진공 서식 다운로드", html_export, f"{safe_name}_중진공.html", "text/html")
