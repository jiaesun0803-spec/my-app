...
if check_password():
    ...
    if st.sidebar.button("📊 AI 기업분석리포트", use_container_width=True): check_and_route("REPORT")
    if st.sidebar.button("💡 AI 정책자금 매칭리포트", use_container_width=True): check_and_route("MATCHING")
    if st.sidebar.button("📝 융자·사업계획서 생성", use_container_width=True): check_and_route("PLAN")
    if st.sidebar.button("📑 AI 사업계획서", use_container_width=True): check_and_route("FULL_PLAN")
    ...
    if st.session_state["view_mode"] == "REPORT":
        ...
        st.title("📋 AI기업분석리포트")
        st.subheader(f"📌 분석 대상 기업: {v.c_name}")
...
<p>(단기/중기/장기 비전 방대한 서술)</p>"""
    ...
    # ==========================================
        ...
        v = get_common_vars(st.session_state["permanent_data"])
        st.title("🎯 AI 정책자금 최적화 매칭")
        st.subheader(f"📌 대상 기업: {v.c_name}")
        ...
        if v.tax_status == '유' or v.fin_status == '유': st.error("🚨 세금체납 및 금융연체가 있을 시 자금 매칭이 진행되지 않습니다.")
            ...
            if "generated_matching" not in st.session_state:
                with st.status("🚀 매칭 심사 중...", expanded=True) as status:
                    st.write("⏳ 로직 적용 중...")
                    try:
                        prompt = f"""당신은 컨설턴트. 마크다운 금지. HTML 표로 결과 출력.
[기업]명:{v.c_name}/업종:{v.c_ind}/근로자:{v.emp_cnt}/매출:{v.s_25}/기대출:{v.tot_debt}/인증:{v.cert_status}/특허:{v.pat_str}
[룰] 보증상태({v.guarantee}) 반영 중복 금지. 1~4순위 추천 자금 표기.
<h2 style="color:#174EA6; border-bottom:2px solid #174EA6;">1. 기업 진단 요약</h2>
<p>매출: <b>{v.s_25}</b> | 기대출: <b style="color:red;">{v.tot_debt}</b> | 희망자금: <b>{v.req_fund}</b></p>
<h2 style="color:#174EA6; border-bottom:2px solid #174EA6;">2. 추천 자금 (1~4순위)</h2>
<p>(1~4순위 자금명, 예상한도, 추천사유를 표 형태로 간결히 작성)</p>
<h2 style="color:#174EA6; border-bottom:2px solid #174EA6;">3. 보완 가이드</h2>
<p>(인증/특허 조언)</p>"""
                        model = genai.GenerativeModel(get_best_model_name())
    ...
    # ==========================================
        ...
        st.markdown("<br><p style='color:#c62828; font-weight:bold;'>※ 필수정보를 입력해야 리포트가 생성됩니다.</p>", unsafe_allow_html=True)
        ...
        with r3c2:
            ls = st.radio("사업장 임대", ["자가", "임대"], horizontal=True, key="in_lease_status")
            if ls == "임대":
        ...
        with r3c3:
            ab = st.radio("추가사업장", ["무", "유"], horizontal=True, key="in_has_additional_biz")
        ...
        with r6c2:
            ...
            st.multiselect("부동산", ["아파트", "빌라", "토지", "임야", "공장", "기타"], key="in_real_estate")
        ...
        with cr2: st.info(f"🏆 KCB: {get_credit_grade(kcb, 'KCB')}등급 | NICE: {get_credit_grade(nice, 'NICE')}등급")

        st.markdown("<br>", unsafe_allow_html=True)
        st.header("* 4. 매출현황")
        m1, m2, m3, m4 = st.columns(4)
        with m1: st.number_input("* 금년(당월)", value=None, step=1, format="%d", placeholder="예:1억=10000", key="in_sales_current")
        with m2: st.number_input("* 25년", value=None, step=1, format="%d", placeholder="예:1억=10000", key="in_sales_2025")
        with m3: st.number_input("* 24년", value=None, step=1, format="%d", placeholder="예:1억=10000", key="in_sales_2024")
        with m4: st.number_input("* 23년", value=None, step=1, format="%d", placeholder="예:1억=10000", key="in_sales_2023")
        
        if st.session_state.get("in_industry", "기타") in ["제조업", "도소매업"]:
            if st.radio("수출 유무", ["무", "유"], horizontal=True, key="in_is_export") == "유":
                e1, e2, e3 = st.columns(3)
                with e1: st.number_input("금년 수출", value=None, step=1, format="%d", key="in_exp_current")
                with e2: st.number_input("25년 수출", value=None, step=1, format="%d", key="in_exp_2025")
                with e3: st.number_input("24년 수출", value=None, step=1, format="%d", key="in_exp_2024")
        ...
        with d4: st.number_input("* 신보(만)", value=None, step=1, format="%d", key="in_debt_kodit")
        d5, d6, d7, d8 = st.columns(4)
        with d5: st.number_input("* 기보(만)", value=None, step=1, format="%d", key="in_debt_kibo")
        with d6: st.number_input("* 기타(만)", value=None, step=1, format="%d", key="in_debt_etc")
        with d7: st.number_input("* 신용(만)", value=None, step=1, format="%d", key="in_debt_credit")
        with d8: st.number_input("* 담보(만)", value=None, step=1, format="%d", key="in_debt_coll")
        ...
        st.markdown("<br>", unsafe_allow_html=True)
        st.header("7. 인증 및 특허")
        for i, cert in enumerate(["소상공인확인서", "창업확인서", "여성기업", "이노비즈", "벤처", "뿌리기업", "ISO", "HACCP"]):
            if st.checkbox(cert, key=f"in_chk_{i}"): st.text_input(f"↪ {cert} 일자", key=f"in_cert_date_{i}", on_change=cb_format_date, args=(f"in_cert_date_{i}",))
        
        pc1, pc2 = st.columns(2)
        with pc1:
            if st.radio("특허", ["무", "유"], horizontal=True, key="in_has_patent") == "유":
                for pt in ["특허출원", "특허등록", "상표등록", "디자인등록"]:
                    c = st.number_input(f"➤ {pt} 건수", min_value=0, step=1, format="%d", key=f"in_{pt}_cnt")
                    for i in range(int(c)): st.text_input(f" - 번호 {i+1}", key=f"in_{pt}_num_{i}")
        with pc2:
        ...
        st.text_area("* [계획]", key="in_future_plan")
