...
if st.session_state["view_mode"] == "INPUT":
    ...
    with c1r2[2]: st.selectbox("업종", ["제조업", "서비스업", "IT업", "도소매업", "건설업", "기기기타"], key="in_industry")
    ...
    # --- 3. 신용 정보 시각화 (수평 칼정렬 4단 구조) ---
    st.markdown("<br>", unsafe_allow_html=True)
    st.header("3. 신용 정보 시각화")
    
    # 3분할 (입력뭉치 | 코멘트박스 | 그래프뭉치)
    ...
    with c3_col1:
        ...
        # 1층: 제목
        st.markdown("<p style='font-size:0.9em; margin-bottom: 2px; font-weight: bold;'>금융연체 여부 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 세금체납 여부</p>", unsafe_allow_html=True)
        # 2층: 라디오 (label 삭제)
        ci_r1 = st.columns(2)
        with ci_r1[0]: delinquency = st.radio("f_delin", ["무", "유"], horizontal=True, key="in_fin_delinquency", label_visibility="collapsed")
        with ci_r1[1]: tax_delin = st.radio("t_delin", ["무", "유"], horizontal=True, key="in_tax_delinquency", label_visibility="collapsed")
        
        # 3층: 제목
        st.markdown("<div style='margin-top: 15px;'></div>", unsafe_allow_html=True)
        st.markdown("<p style='font-size:0.9em; margin-bottom: 2px; font-weight: bold;'>KCB 점수 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; NICE 점수</p>", unsafe_allow_html=True)
        # 4층: 입력창 (label 삭제 및 너비 축소)
        ci_r2 = st.columns(2)
        with ci_r2[0]: s_kcb = st.number_input("k_in", value=0, max_value=1000, key="in_kcb_score", label_visibility="collapsed")
        with ci_r2[1]: s_nice = st.number_input("n_in", value=0, max_value=1000, key="in_nice_score", label_visibility="collapsed")
        
    with c3_col2:
        # 가운데: 신용 코멘트 박스 (우측 그래프 높이와 일치)
        st.markdown("<div style='margin-top: 10px;'></div>", unsafe_allow_html=True)
        if delinquency == "유" or tax_delin == "유":
            st.markdown(f"""
            <div style='background-color: #FFEBEE; padding: 20px; border-radius: 10px; border-left: 5px solid #E53935; height: 175px;'>
                <h3 style='color: #B71C1C; margin-top: 0;'>⚠️ 연체/체납 주의</h3>
                <p style='color: #D32F2F; font-size: 0.9em; line-height: 1.5;'>현재 연체 또는 체납 정보가 확인됩니다. 자금 신청 시 결격사유가 될 수 있으므로 즉시 해결이 필요합니다.</p>
            </div>
            """, unsafe_allow_html=True)
        else:
            st.markdown(f"""
            <div style='background-color: #E8F5E9; padding: 20px; border-radius: 10px; border-left: 5px solid #43A047; height: 175px;'>
    ...
    st.success("✅ [정렬 완료] 모든 데이터가 복구되었으며 수평 라인이 일치하도록 세팅되었습니다.")
