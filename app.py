...
if st.sidebar.button("💾 정보 저장", use_container_width=True):
    ...
    if name:
        ...
        db[name] = current_data; save_db(db); st.sidebar.success(f"{name} 저장 완료!")
...
if st.session_state["view_mode"] == "INPUT":
    ...
    with c3_col2:
        ...
        elif vk > 700: status, color, text = "🟢 진행 원활", "#E8F5E9", "양호한 신용 상태"
        else: status, color, text = "🟡 진행 주의", "#FFF3E0", "신용 보완 권장"
    ...
    with c7[0]:
        ...
        st.number_input("보유 건수", key="in_pat_cnt", step=1, placeholder="0"); st.text_area("특허 상세 내용", key="in_pat_desc", placeholder="특허 명칭 및 주요 내용")
    with c7[1]:
        st.radio("정부지원 수혜이력", ["없음", "있음"], horizontal=True, key="in_has_gov")
        st.number_input("수혜 건수", key="in_gov_cnt", step=1, placeholder="0"); st.text_area("수혜 사업명 상세", key="in_gov_desc", placeholder="과거 선정 사업명")
    st.markdown("---")

    # --- 8. 비즈니스 상세 정보 ---
    st.header("8. 비즈니스 상세 정보")
    r8_1 = st.columns(2); r8_1[0].text_area("핵심 아이템", key="in_item_desc", height=100, placeholder="제품/서비스 핵심 기능"); r8_1[1].text_area("판매 루트(유통망)", key="in_sales_route", height=100, placeholder="주요 거래처 및 채널")
    r8_2 = st.columns(2); r8_2[0].text_area("경쟁력 및 차별성", key="in_item_diff", height=100, placeholder="타사 대비 강점"); r8_2[1].text_area("시장 현황", key="in_market_status", height=100, placeholder="현재 업계 분위기")
    r8_3 = st.columns(2); r8_3[0].text_area("공정도", key="in_process_desc", height=100, placeholder="생산 및 제공 과정 요약"); r8_3[1].text_area("타겟 고객", key="in_target_cust", height=100, placeholder="주요 구매 및 이용자층")
    r8_4 = st.columns(2); r8_4[0].text_area("수익 모델", key="in_revenue_model", height=100, placeholder="매출이 발생하는 구조"); r8_4[1].text_area("앞으로의 계획", key="in_future_plan", height=100, placeholder="향후 비즈니스 로드맵")
    st.markdown("---")
...
# ==========================================
    ...
    if not st.session_state.get("api_key"):
        ...
        with st.status(f"🚀 {biz_name} AI 리포트 생성 중..."):
            try:
