...
<style>
    ...
    /* 2. 섹션 헤더 스타일 */
    h2 { font-weight: 700 !important; margin-top: 20px !important; }
...
# 불러오기 버튼을 상단으로 배치 (좌측 정렬 느낌을 위해 컬럼 활용)
...
if st.session_state["view_mode"] == "INPUT":
    ...
    with c3_col2:
        ...
        status_text = "⚠️ 해결 필요" if delinquency == "유" or tax_delin == "유" else "✅ 신용 양호"
        st.markdown(f"""<div style='background-color:{box_color}; padding:20px; border-radius:10px; height:185px;'><p style='font-weight:700;'>{status_text}</p><p style='font-size:0.9em;'>기초 요건을 바탕으로 최적의 정책자금을 분석합니다.</p></div>""", unsafe_allow_html=True)
    with c3_col3:
    ...
    for row in range(0, 8, 4):
        ...
        for i in range(4):
            ...
            with cols[i]:
                st.markdown(f"{name} {LABEL_SPAN}", unsafe_allow_html=True)
            ...
            with st.spinner("AI가 전략 구상 중..."):
                try:
                    ...
                    for k, v in res_json.items():
                        mapping = {"차별성":"in_item_diff", "시장현황":"in_market_status", "수익모델":"in_revenue_model", "판매루트":"in_sales_route", "공정도":"in_process_desc", "타겟고객":"in_target_cust", "계획":"in_future_plan"}
    ...
    c9 = st.columns([1, 2])
    with c9[0]:
        st.markdown(f'<p class="blue-bold-label">**이번 조달 필요 자금 (만원)**</p>', unsafe_allow_html=True)
    ...
    st.success("✅ [디자인 업데이트 완료] 구분선 삽입, 6번 폰트 확대 및 9번 강조가 적용되었습니다.")
